...
public class PickUpItemTool(ToolContext ctx)
{
    ...
    public async Task<string> PickUpItem(
        ...
        string description,
        [Description("The item's game effect. Use one of: 'heal:N' (restores N health points when used), 'food:N' (reduces hunger by N when eaten), 'weapon:N' (equippable — N attack power), 'armor:N' (equippable — N damage reduction), 'poison:N' (dangerous — deals N damage on contact and is NOT stored). Use empty string for items with no mechanical effect.")]
        string effect,
